...
from models import Node, Route, Worker
...
def parse_workers(text: str, veh_count: int) -> List[Worker]:
    ...
    Returns:
        List of Worker objects with name and skills
    """
    workers = []
    if text:
...
def assign_workers(routes: List[Route], nodes: Dict[int, Node], workers: List[Worker]) -> Dict[int, str]:
    """
    ...
    for r in routes:
        ...
        if idx is not None:
            ...
            worker = Worker(r.vehicle_id, f"Worker {r.vehicle_id}", set())
